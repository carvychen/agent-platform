...
- **Client-side is independent.** The agent consumes this server through `agent_framework.MCPStreamableHTTPTool` (the same Python MCP SDK on the client side, ADR 0005). The server-side SDK choice is orthogonal to whichever client (AF's MCP tool, Claude Desktop, VS Code Copilot MCP, Copilot Studio, or a custom client) an external agent chooses — they all speak the same Streamable HTTP protocol.
